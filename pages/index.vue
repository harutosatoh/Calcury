  ...
  <div class="container">
    ...
    <div id="table">
      ...
      <div class="p-title-container">
        ...
        <h3 class="sum">
          ￥{{ divider(this.$store.state.currentSum[0].number) }}
        </h3>
      </div>
      <div v-for="(item, index) in users" :key="index">
        <div class="card">
          ...
          />
          <button
            class="button is-primary is-light card-content is-rounded"
            @click="insertPayment(item.uid)"
          >
            支払いを追加
          </button>
        </div>
  ...
  methods: {
    insertUser1: function () {
      ...
      if (this.first !== '') {
        this.$store.commit(
          'insertUser',
          { name: this.first, uid: 1, payment: '0' },
          { root: true }
        )
    ...
    insertUser2: function () {
      ...
      if (this.second !== '') {
        this.$store.commit(
          'insertUser',
          { name: this.second, uid: 2, payment: '0' },
          { root: true }
        )
    ...
    insertUser3: function () {
      ...
      if (this.third !== '') {
        this.$store.commit(
          'insertUser',
          { name: this.third, uid: 3, payment: '0' },
          { root: true }
        )
        console.log(this.first)
        this.isPush3 = true
    ...
    insert: function () {
      if (this.content !== '' && this.sum !== null) {
        ...
        this.$store.commit(
          'insert',
          { number: this.sum, content: this.content },
          { root: true }
        )
    ...
    insertPayment: function (uid) {
      this.$store.commit(
        'paymentsEach',
        { number: this.paymentseach, uid: uid },
        { root: true }
      )
      this.paymentseach = ''
